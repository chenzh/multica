...
site_factory_write_mvp_fallback() {
  ...
  mkdir -p "$delivery_dir"
  cp "$example/brief.md" "$example/accept_cases.md" "$example/backlog.md" "$delivery_dir/" 2>/dev/null || true
...
if needle in text:
    text = text.replace(
        "1. 海外用户可用的 **单页工具**（示例：JSON 格式化器）营销 + 工具合一页。",
        f"1. **{idea}** — minimal Cloudflare Pages demo.",
        1,
    )
